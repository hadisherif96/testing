...
Usage:
  python scrapper/product_scraper_v3.py "https://example.com" \
...
def _extract_product_data(page: Page) -> ProductData:
            ...
            try:
                ...
                try:
                    ...
                    if is_product:
                        ...
                        product = _extract_product_data(new_page)
    ...
    with sync_playwright() as p:
        ...
        while to_visit and len(visited_urls) < max_pages:
            ...
            try:
                ...
                if is_product:
                    # Extract product data
                    product = _extract_product_data(page)
        ...
        buy_button_products = sum(len(r.products) for r in results if not r.is_product_page and r.products)
        
        print(f"\n{'='*60}")
        print(f"[Summary] Crawl Complete")
        print(f"{'='*60}")
        print(f"  📄 Pages crawled: {len(visited_urls)}")
        print(f"  🛍️  Product pages found: {product_pages}")
        print(f"  🔘 Products from 'Buy' buttons: {buy_button_products}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Product Scraper V3 - Enhanced with page scrolling for complete content loading"
    )
    ...
    print(f"\n{'='*60}")
    print(f"Product Scraper V3 - Enhanced with Page Scrolling & Buy Button Detection")
    print(f"{'='*60}")
    print(f"Features:")
